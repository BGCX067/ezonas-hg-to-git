...
void			VertexIndexToShape::addStaticVertexData(const VertexData *vertex_data)
{
	...
	Vec3* tmp_vert = new Vec3[mVertexCount];
	if (mVertexBuffer)
	{
		memcpy(tmp_vert, mVertexBuffer, sizeof(Vec3) * prev_size);
	...
	{	
		...
		float* pReal;
		Vec3 * curVertices = &mVertexBuffer[prev_size];
...
void			VertexIndexToShape::addAnimatedVertexData
	(const Ogre::VertexData *vertex_data, const Ogre::VertexData *blend_data,
...
Vec3			VertexIndexToShape :: getSize()
{
	const unsigned int vCount = getVertexCount();
	if (mBounds == Vec3(-1,-1,-1) && vCount > 0)
	{

		const Vec3 * const v = getVertices();
...
const Vec3*		VertexIndexToShape :: getVertices() { return mVertexBuffer; }
unsigned int	VertexIndexToShape :: getVertexCount()
{
	return mVertexCount;
}
const unsigned int* VertexIndexToShape :: getIndices() { return mIndexBuffer; }
unsigned int	VertexIndexToShape :: getIndexCount() { return mIndexCount; }
...
btConvexHullShape*	VertexIndexToShape :: createConvex()
{
	...
	return new btConvexHullShape((btScalar*) &mVertexBuffer[0].x, mVertexCount, sizeof(Vec3));
}
btBvhTriangleMeshShape* VertexIndexToShape :: createTrimesh()
{
	...
	Vec3 *vertices = mVertexBuffer;

	btVector3    vertexPos[3];
	for (unsigned int n = 0; n < numFaces; ++n)
	{
		{
			const Vec3 &vec = vertices[*indices];
			vertexPos[0][0] = vec.x;
			vertexPos[0][1] = vec.y;
			vertexPos[0][2] = vec.z;
		}
		{
			const Vec3 &vec = vertices[*(indices + 1)];
			vertexPos[1][0] = vec.x;
			vertexPos[1][1] = vec.y;
			vertexPos[1][2] = vec.z;
		}
		{
			const Vec3 &vec = vertices[*(indices + 2)];
			vertexPos[2][0] = vec.x;
			vertexPos[2][1] = vec.y;
			vertexPos[2][2] = vec.z;
		}
...
void StaticMeshToShapeConverter::addEntity(Entity *entity,const Matrix4 &transform)
{
	...
	mTransform = transform;
	mScale = mNode ? mNode->getScale() : Vec3(1,1,1);
...
bool			AnimatedMeshToShapeConverter :: getBoneVertices
	(unsigned char bone, unsigned int &vertex_count,  Vec3* &vertices,
	const Vec3 &bonePosition)
{
	...
	{	
		...
		mTransformedVerticesTemp = new Vec3[vertex_count];
	...
	Vec3Array::iterator j = i->second->begin();
...
btBoxShape*		AnimatedMeshToShapeConverter :: createAlignedBox
	(unsigned char bone, const Vec3 &bonePosition, const Quaternion &boneOrientation)
{
	...
	Vec3 min_vec(vertices[0]);
	Vec3 max_vec(vertices[0]);
	...
	const Vec3 maxMinusMin(max_vec - min_vec);
...
bool AnimatedMeshToShapeConverter::getOrientedBox
	(unsigned char bone, const Vec3 &bonePosition, const Quaternion &boneOrientation,
	Vec3 &box_afExtent, Vec3 *box_akAxis, Vec3 &box_kCenter)
{
	unsigned int vertex_count;
	Vec3* vertices;
	...
	Vec3 kDiff (vertices[1] - box_kCenter);
...
void	DynamicRenderable :: initialize
	(RenderOperation::OperationType operationType, bool useIndices)
{
...
RenderOperation::OperationType
			DynamicLines :: getOperationType() const
{
...
const Vec3& DynamicLines :: getPoint(unsigned short index) const
{
...
void		DynamicLines :: setPoint(unsigned short index, const Vec3 &value)
{
	assert(index < mPoints.size() && "Point index is out of bounds!!");
...
void		DynamicLines :: fillHardwareBuffers()
{
	...
	Vec3 vaabMin = mPoints[0];
	Vec3 vaabMax = mPoints[0];
