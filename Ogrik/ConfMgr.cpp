...
SceneNode * ConfMgr :: FastAdd(string _s)
{
	...
	SceneNode * node = Application :: GetScMgr() -> createSceneNode(_s);
	Entity * ent = Application :: GetScMgr() -> createEntity(mesh_filename);
	if (material_name != "-") ent -> setMaterialName(material_name);
	node -> attachObject(Application :: GetScMgr() -> createEntity(mesh_filename));
	((Application :: GetScMgr()) -> getRootSceneNode()) -> addChild(node);
